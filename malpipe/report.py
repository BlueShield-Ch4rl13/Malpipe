...
def _section_perms(s):
    if s["exec"] and s["write"]:
        return "RWX"
    return "X" if s["exec"] else ""
...
def write_html(out_dir: Path, result: dict) -> Path:
    ...
    reasons = "".join(
        ...
        f'<span>{_esc(r["label"])}</span></div>'
        for r in sc["reasons"]
    ) or '<div class="muted">Sin señales de riesgo.</div>'

    cap_rows = [
        [
            f'<span class="mono att">{_esc(c["attack"])}</span>',
            _esc(c["capability"]),
            _esc(c["tactic"]),
            f'<span class="muted mono">{_esc(", ".join(c["evidence"]))}</span>',
        ]
        for c in caps
    ]
    caps_html = (
        _rows(["ATT&CK", "Capacidad", "Táctica", "Evidencia"], cap_rows)
        if caps
    ...
    yara_rows = [
        [
            f'<span class="mono">{_esc(m["rule"])}</span>',
            _esc((m.get("meta", {}) or {}).get("severity", "")),
            _esc((m.get("meta", {}) or {}).get("description", "")),
        ]
        for m in yara
    ]
    yara_html = (
        _rows(["Regla", "Severidad", "Descripción"], yara_rows)
        if yara
    ...
    ioc_rows = [
        [
            _esc(i["type"]),
            f'<span class="mono wrap">{_esc(i.get("defanged", i["value"]))}</span>',
        ]
        for i in iocs
    ]
    iocs_html = (
        _rows(["Tipo", "Indicador"], ioc_rows)
        if iocs
    ...
    if pe.get("sections"):
        sec_rows = [
            [
                f'<span class="mono">{_esc(s["name"])}</span>',
                _esc(s["vsize"]),
                _esc(s["rsize"]),
                f'<span class="{"hot" if s["entropy"] >= 7.2 else ""}">'
                f'{_esc(s["entropy"])}</span>',
                _section_perms(s),
            ]
            for s in pe["sections"]
        ]
        sec_headers = ["Sección", "V.Size", "R.Size", "Entropía", "Perm."]
        sec_html = f"<h3>Secciones del PE</h3>{_rows(sec_headers, sec_rows)}"
    ...
    anoms_html = ""
    if anoms:
        items = "".join(f"<li>{_esc(a)}</li>" for a in anoms)
        anoms_html = f"<h3>Anomalías estructurales</h3><ul class='an'>{items}</ul>"
    ...
    if result.get("strings"):
        ...
        joined = _esc("\n".join(interesting))
        strings_preview = (
            "<h3>Cadenas relevantes (muestra)</h3>"
            '<div class="scroll" style="max-height:300px">'
            f'<pre class="mono">{joined}</pre></div>'
        )

    doc = TEMPLATE
    replacements = {
        "TITLE": _esc(idn["filename"]),
        "COLOR": col,
        "VERDICT": _esc(sc["verdict"].upper()),
        "SCORE": _esc(sc["risk_score"]),
        "CONF": _esc(sc["confidence"]),
        "FMT": _esc(idn.get("format", "")),
        "TYPE": _esc(idn.get("type", "")),
        "SIZE": _esc(idn["size"]),
        "ENT": _esc(idn["entropy"]),
        "MD5": _esc(idn["md5"]),
        "SHA256": _esc(idn["sha256"]),
        "IMPHASH": _esc(pe.get("imphash", "") or "&mdash;"),
        "SSDEEP": _esc(idn.get("ssdeep", "")),
...
.mono{font-family:var(--mono);font-size:12.5px}.muted{color:var(--muted)}
...
.hero{display:grid;grid-template-columns:170px 1fr;gap:22px;background:var(--panel);
border:1px solid var(--line);border-radius:12px;padding:20px;margin-bottom:18px}
.gauge{display:flex;flex-direction:column;align-items:center;justify-content:center}
.gauge .v{font-family:var(--mono);font-size:13px;text-align:center;margin-top:6px;
font-weight:600}
...
.card h3,.card h2{margin:0 0 12px;font-size:13px;text-transform:uppercase;
letter-spacing:.06em;color:var(--muted)}
h3{margin:18px 0 10px;font-size:13px;text-transform:uppercase;letter-spacing:.06em;
color:var(--muted)}
.reason{display:flex;gap:10px;align-items:center;padding:6px 0;
border-bottom:1px solid var(--line)}
.reason:last-child{border:none}
.reason .pts{font-family:var(--mono);color:var(--amber);min-width:38px}
.scroll{overflow:auto;border:1px solid var(--line);border-radius:8px}
table{border-collapse:collapse;width:100%;font-size:12.5px}
thead th{position:sticky;top:0;background:#0F151D;text-align:left;padding:8px 12px;
...
pre{margin:0;padding:12px;white-space:pre-wrap;word-break:break-all;font-size:11.5px;
color:var(--muted)}
footer{color:var(--muted);font-size:12px;padding:18px 26px;
border-top:1px solid var(--line);text-align:center}
</style></head><body>
<header><span class="logo">&#9679; malpipe</span>
<span class="muted">análisis estático de malware</span></header>
<main>
  <div class="hero">
    <div class="gauge">
      <svg width="150" height="150" viewBox="0 0 150 150">
        <circle cx="75" cy="75" r="64" fill="none" stroke="#171E28" stroke-width="13"/>
        <circle cx="75" cy="75" r="64" fill="none" stroke="{{COLOR}}" stroke-width="13"
          stroke-linecap="round" stroke-dasharray="402"
          stroke-dashoffset="calc(402 - 402 * {{SCORE}} / 100)"
          transform="rotate(-90 75 75)"/>
        <text x="75" y="70" text-anchor="middle" fill="{{COLOR}}"
          font-family="monospace" font-size="34" font-weight="700">{{SCORE}}</text>
        <text x="75" y="92" text-anchor="middle" fill="#8B97A5"
          font-family="monospace" font-size="11">/ 100</text>
      </svg>
    ...
    <div>
      <div class="facts">
        ...
        <div><div class="k">Formato · tipo</div>
          <div class="val">{{FMT}} · {{TYPE}}</div></div>
        ...
        <div style="grid-column:1/3"><div class="k">SHA-256</div>
          <div class="val">{{SHA256}}</div></div>
        <div><div class="k">MD5</div><div class="val">{{MD5}}</div></div>
        <div><div class="k">imphash</div><div class="val">{{IMPHASH}}</div></div>
        <div style="grid-column:1/3"><div class="k">ssdeep</div>
          <div class="val">{{SSDEEP}}</div></div>
      </div>
    </div>
  </div>
  <div class="grid">
...
<footer>malpipe · generado {{GENERATED}} · análisis estático · uso exclusivamente
defensivo. La parte dinámica se ejecuta en un lab aislado.</footer>
</body></html>"""
