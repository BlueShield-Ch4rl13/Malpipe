...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--output",
        type=Path,
        help="Carpeta de salida (por defecto malpipe_<sha>)",
    )
    ap.add_argument(
        "--rules",
        type=Path,
    ...
    ap.add_argument(
        "--dynamic",
        action="store_true",
        help="Ejecutar análisis dinámico (sandbox aislado)",
    )
    ...
    sc = result["score"]
    print(
        f"  Veredicto : {sc['verdict']} · riesgo {sc['risk_score']}/100 "
        f"· confianza {sc['confidence']}"
    )
